...
class NNTags
{
	public static function getValuesFromString($string = '', $main_key = 'title', $known_boolean_keys = array())
	{
		...
		$temp_quote = '[[QUOTE]]';
		// replace escaped quotes with special markup
		$string = str_replace('\\"', $temp_quote, $string);
		...
		foreach ($values['1'] as $i => $key)
		{
			...
			// replace back the special quote markup to normal quotes
			$value = str_replace($temp_quote, '"', $value);
